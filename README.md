...
**Experimental v0.1.0. Software tests are included; a real T-Dongle-S3 and proxy installation must be tested before relying on detections. This project does not operate doors and includes no garage automation or blueprint.**
...
- Per-car recent-authentication sensor, timestamp, authentication event and test button.
...
The initial implementation passes 27 automated tests covering protocol framing, replay rejection, Python/C++ message agreement, GATT authentication failures and cleanup, resumable configuration, and credential revocation. The ESP32-S3 firmware compiles with ESPHome 2026.8.2. These checks run locally against Home Assistant 2026.9.1 libraries. No live Home Assistant instance or physical dongle has been modified or tested.
